...
- `[project.scripts]` 中的 `sing = "sing_cli.main:main"` 指向 `src/sing_cli/main.py` 内的 `main()`。
...
本地 profiles 和索引存放在 `typer.get_app_dir("sing-cli")` 返回的应用目录中：
...
- `active` 表示最后一次成功写入 Windows 服务命令行并启动的 profile 名。
- `profiles` 按 profile 名索引 URL、本地文件路径和更新时间。
- `profiles/<name>` 保存 URL 下载到的完整 `sing-box` JSON profile，不自动追加文件扩展名。

## 命名规则

- Windows 服务名固定为 `sing-box`。
- profile 名称用于文件名，只允许字母、数字、下划线、短横线和点。
