...
def response_with_status(status_code: int) -> httpx.Response:
    request = httpx.Request("GET", "https://example.com/profile.json")
...
def test_download_profile_reports_http_error(tmp_path: Path) -> None:
    destination = tmp_path / "profile.json"

    with pytest.raises(SingCliError, match="Unable to download profile"):
        download_profile(
            "https://example.com/profile.json",
            destination,
            StubHttpClient(response_with_status(500)),
        )
